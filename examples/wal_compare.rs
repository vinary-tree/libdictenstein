...
fn main() {
    for size in [100, 1000, 10000] {
        ...
        println!("  Individual: {} bytes, {} WAL records ({:.1} bytes/term)", 
                 individual_size, individual_records, individual_size as f64 / size as f64);
        println!("  Batch:      {} bytes, {} WAL record(s) ({:.1} bytes/term)", 
                 batch_size, batch_records, batch_size as f64 / size as f64);
        
        let size_diff = batch_size as i64 - individual_size as i64;
        if size_diff > 0 {
            println!("  Result: Batch is {} bytes LARGER ({:.1}% larger)", 
                     size_diff, (size_diff as f64 / individual_size as f64) * 100.0);
        } else {
            println!("  Result: Batch is {} bytes SMALLER ({:.1}% reduction)", 
                     -size_diff, (-size_diff as f64 / individual_size as f64) * 100.0);
        }
        println!();
    }
