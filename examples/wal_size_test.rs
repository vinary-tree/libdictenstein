...
fn main() {
    ...
    println!();
    println!(
        "Expected header overhead (individual): {} bytes",
        individual_header_overhead
    );
    println!(
        "Expected header overhead (batch):      {} bytes",
        batch_header_overhead
    );
    println!(
        "Header overhead reduction:             {:.1}%",
