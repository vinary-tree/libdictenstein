...
fn main() {
    ...
    println!(
        "Target trie before merge: {} terms",
    ...
    let mut regular: PersistentARTrie<i64> =
        PersistentARTrie::create(&path3).expect("create regular");
    ...
    println!(
        "Regular trie after merge: {} terms",
        regular.len().unwrap_or(0)
    );

    // Test 2: Batched merge with batch_size=100
    let dir4 = tempdir().expect("create temp dir");
    let path4 = dir4.path().join("batched.artrie");
    let mut batched: PersistentARTrie<i64> =
        PersistentARTrie::create(&path4).expect("create batched");

    for i in 500..1500 {
        batched.insert_with_value(&format!("term_{:08}", i), (i * 10) as i64);
    }

    let batched_count = batched
        .merge_from_batched(&source, |a, b| a + b, 100)
        .expect("batched merge");
    println!(
        "\nBatched merge (batch_size=100): {} terms processed",
        batched_count
    );
    println!(
        "Batched trie after merge: {} terms",
        batched.len().unwrap_or(0)
    );
    ...
    println!(
        "term_00000000: regular={:?}, batched={:?}",
        val1_regular, val1_batched
    );
    ...
    println!(
        "term_00001200: regular={:?}, batched={:?}",
        val2_regular, val2_batched
    );
    ...
    println!(
        "term_00000600 (overlap): regular={:?}, batched={:?}",
        val3_regular, val3_batched
    );
