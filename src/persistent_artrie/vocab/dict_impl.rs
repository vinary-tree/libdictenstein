...
//!
//! This module provides the core disk-backed vocabulary trie implementation. The only live
//! representation is the lock-free overlay; reverse lookups use the in-memory
//! `reverse_term_map`, which is rebuilt from the checkpoint image and WAL tail on reopen.
//! It is built on the base persistence infrastructure from `persistent_artrie`
//! (WAL, BufferManager, ArenaManager).
//!
//! # Architecture
//!
//! ```text
...
//! │                                                              │
//! │  Files:                                                      │
//! │  - vocabulary.vocab      # Dense overlay checkpoint image   │
...
//! └─────────────────────────────────────────────────────────────┘
//! │ Dense char-overlay checkpoint image                         │
//! └─────────────────────────────────────────────────────────────┘
//! ```
...
/// Persistent vocabulary ARTrie backed by a lock-free overlay.
///
...
///
/// // Reverse lookup (via reverse_term_map)
...
pub struct PersistentVocabARTrie<S: BlockStorage = MmapDiskManager> {
    ...
    pub(crate) epoch_manager: Arc<crate::persistent_artrie::core::concurrency::EpochManager>,
...
// Safety: all mutable shared state is synchronized internally. The overlay root
// uses atomic Arc publication, forward/reverse caches are DashMap-backed, scalar
// counters are atomics, storage handles are behind Arc<RwLock<_>>, and S is
// constrained by BlockStorage: Send + Sync + 'static.
...
mod tests {
    ...
    fn test_forward_lookup() {
        ...
        let vocab = PersistentVocabARTrie::create(&path).unwrap();
        vocab.insert("apple").expect("insert term failed");
        vocab.insert("banana").expect("insert term failed");
        vocab.insert("cherry").expect("insert term failed");
    ...
    fn test_checkpoint_reopen_modify_checkpoint() {
        ...
        // Phase 2: Reopen, insert more, checkpoint
        {
            ...
            assert_eq!(vocab.len(), 2);
            vocab.insert("cherry").expect("insert term failed");
            vocab.insert("durian").expect("insert term failed");
            vocab.checkpoint().unwrap();
        }
    ...
    fn test_contains() {
        ...
        let vocab = PersistentVocabARTrie::create(&path).unwrap();
        vocab.insert("present").expect("insert term failed");
    ...
    fn test_wal_recovery() {
        ...
        {
            let vocab = PersistentVocabARTrie::create(&path).unwrap();
            vocab.insert("term1").expect("insert term failed");
            vocab.insert("term2").expect("insert term failed");
            vocab.insert("term3").expect("insert term failed");
    ...
    /// Regression test for trie growth during load plus exact reverse-map rebuild.
    ///
    ...
    /// lookups are rebuilt from the overlay image.
    #[test]
    fn test_regression_node_growth_during_load() {
        ...
        // Reopen and verify all terms are present
        {
            ...
            // Verify reverse lookups through the rebuilt reverse_term_map.
            for (i, term) in terms.iter().enumerate() {
                assert_eq!(
    ...
    fn test_regression_value_preservation() {
        ...
        // Create terms and verify their values (indices) are preserved
        {
            ...
            vocab.insert("with_value_1").expect("insert term failed");
            vocab.insert("with_value_2").expect("insert term failed");
            vocab.insert("with_value_3").expect("insert term failed");
    ...
    fn test_sync_to_disk_async_non_blocking() {
        ...
        vocab.write().insert("world").expect("insert term failed");
    ...
    /// independently round-trips "" through the overlay image and rebuilt
    /// reverse_term_map. Real-disk scratch (`target/test-tmp`), not tmpfs.
    #[test]
    fn test_empty_string_survives_checkpoint_reopen() {
        ...
        assert_eq!(
            vocab.get_term(0),
            Some("".to_string()),
            "index 0 -> \"\" lost after reopen (reverse map root branch)"
        );
    ...
    fn test_serialization_roundtrip() {
        ...
        // Create and populate
        {
            ...
            vocab.insert("simple").expect("insert term failed");
            vocab.insert("日本語").expect("insert term failed");
            vocab.insert("").expect("insert term failed");
            vocab
                .insert("with spaces and punctuation!")
                .expect("insert term failed");
            vocab.insert(&"x".repeat(100)).expect("insert term failed");
    ...
    fn test_next_index_tracking() {
        ...
        vocab.insert("first").expect("insert term failed");
        assert_eq!(vocab.next_index(), 1);

        vocab.insert("second").expect("insert term failed");
    ...
    fn test_custom_start_index_serialization() {
        ...
        {
            let vocab = PersistentVocabARTrie::create_with_start_index(&path, 1000).unwrap();
            vocab.insert("test").expect("insert term failed");
