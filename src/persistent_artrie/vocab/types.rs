...
//!
//! This module defines the durable header and compatibility constants for the
//! vocabulary-specialized ARTrie. The live representation is the lock-free
//! char overlay at `V = u64`; reverse lookups use the rebuildable
//! `reverse_term_map` rather than an owned node side table.

// Re-export NodeRef from persistent_artrie::char
pub use crate::persistent_artrie::char::types::NodeRef;
...
/// Header format version 1 — the legacy owned image (`root_ptr` = an owned node `ArenaSlot`).
...
/// `enumerate_overlay_terms_from_disk`), not an owned-node `ArenaSlot`. The reverse map is
/// derived and rebuilt in memory on reopen, so `reverse_index_capacity` is 0. Legacy v1
/// owned files are rebuilt, not migrated.
...
pub struct VocabTrieFileHeader {
    ...
    pub next_index: u64,
    /// Historical reverse-index file capacity field; retained for header compatibility.
