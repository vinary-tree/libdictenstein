...
//!
//! Every ctor flips to the lock-free overlay at construction; the owned tree, reverse lookup
//! side table, and bloom filter are deleted. The `IoUringDiskManager` variants live in
...
impl PersistentVocabARTrie {
    ...
    fn open_snapshot<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let header = crate::persistent_artrie::vocab::header::read_vocab_header(&disk_manager)?;
        header.validate()?;
