//! Thread-safe node pointer for CAS-style operations (u8/ASCII variant)
//!
...
//! # Design
//!
//! The pointer stores an `Arc<PersistentNode>` behind a small lock. Earlier
...
/// A CAS-style pointer to a persistent node (u8/byte keys).
///
/// This wrapper enables thread-safe compare-and-swap-style operations on
/// `Arc<PersistentNode>` pointers.
///
/// # Thread Safety
///
...
pub struct AtomicNodePtr {
    ...
    ptr: RwLock<Option<Arc<PersistentNode>>>,
}
...
impl AtomicNodePtr {
    ...
    pub fn compare_exchange(
        ...
        match guard.as_ref() {
            ...
            None => Err(Arc::new(PersistentNode::new())),
        }
    }
