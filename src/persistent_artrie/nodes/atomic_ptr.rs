//! Atomic Node Pointer for Lock-Free CAS Operations (u8/ASCII variant)
//!
//! This module provides `AtomicNodePtr`, a wrapper around `Arc<PersistentNode>`
...
//! For UTF-8/Unicode keys using u32 code points, see `persistent_artrie_char::AtomicNodePtr`.
//!
//! # Design
//!
//! The pointer stores an `Arc<PersistentNode>` as a raw u64 address in an `AtomicU64`.
...
//!
//! let node = Arc::new(PersistentNode::new());
...
//! let new_node = Arc::new(current.with_child(b'a', child));
//!
...
use super::persistent_node::PersistentNode;

/// Null pointer sentinel value (0 is never a valid Arc address)
const NULL_PTR: u64 = 0;

/// An atomic pointer to a persistent node (u8/byte keys).
///
/// This wrapper enables lock-free compare-and-swap operations on
/// `Arc<PersistentNode>` pointers. It's the core building block
/// for lock-free trie modifications.
///
/// # Thread Safety
///
...
pub struct AtomicNodePtr {
    ...
    /// 0 = null, otherwise = Arc<PersistentNode> raw pointer
    ptr: AtomicU64,
}
...
impl AtomicNodePtr {
    ...
    pub fn new(node: Arc<PersistentNode>) -> Self {
    ...
    /// # Returns
    ///
    /// - `Some(Arc<PersistentNode>)` if the pointer is not null
    /// - `None` if the pointer is null
    pub fn load(&self) -> Option<Arc<PersistentNode>> {
        ...
        unsafe {
            let ptr = raw as *const PersistentNode;
    ...
    pub fn store(&self, node: Arc<PersistentNode>) {
        ...
        if old_raw != NULL_PTR {
            unsafe {
                Arc::decrement_strong_count(old_raw as *const PersistentNode);
            }
        }
    }
    ...
    pub fn take(&self) -> Option<Arc<PersistentNode>> {
    ...
    pub fn compare_exchange(
    ...
    ) -> Result<Arc<PersistentNode>, Arc<PersistentNode>> {
            ...
            .compare_exchange(expected_raw, new_raw, Ordering::AcqRel, Ordering::Acquire)
        {
            ...
            Err(actual_raw) => {
                ...
                unsafe {
                    Arc::decrement_strong_count(new_raw as *const PersistentNode);
                }
                ...
                if actual_raw == NULL_PTR {
                    ...
                    Err(Arc::new(PersistentNode::new()))
                } else {
                    unsafe {
                        Arc::increment_strong_count(actual_raw as *const PersistentNode);
                        Err(Arc::from_raw(actual_raw as *const PersistentNode))
                    }
                }
            }
        }
...
impl Drop for AtomicNodePtr {
    fn drop(&mut self) {
        ...
        if raw != NULL_PTR {
            ...
            unsafe {
                drop(Arc::from_raw(raw as *const PersistentNode));
            }
        }
    }
}
...
mod tests {
    ...
    fn test_compare_exchange_failure() {
        ...
        let child = SwizzledPtr::on_disk(1, 100, NodeType::Node4);
        let node2 = Arc::new(node1.with_child(b'a', child));
        let node3 = Arc::new(PersistentNode::new());
    ...
    fn test_try_init_failure() {
        let node1 = Arc::new(PersistentNode::new());
        let node2 = Arc::new(PersistentNode::new());
        let ptr = AtomicNodePtr::new(node1);
    ...
    fn test_clone() {
        let child = SwizzledPtr::on_disk(1, 100, NodeType::Node4);
        let node = Arc::new(PersistentNode::new().with_child(b'a', child));
            ...
            .map(|t| {
                ...
                thread::spawn(move || {
                    ...
                    for i in 0..ops_per_thread {
                        ...
                        let child = SwizzledPtr::on_disk(key as u32, 0, NodeType::Node4);
