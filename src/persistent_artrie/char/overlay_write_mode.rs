...
//! trait in `persistent_artrie::core::overlay::flip`. This module now holds only:
//!
    ...
    for super::PersistentARTrieChar<V, S>
{
    ...
    fn lockfree_root(
        &self,
    ) -> Option<&crate::persistent_artrie::core::overlay::AtomicNodePtr<CharKey, V>> {
    ...
    fn install_prebuilt_overlay_root_seam(
        &mut self,
        root: Arc<crate::persistent_artrie::core::overlay::node::OverlayNode<CharKey, V>>,
    ) {
    ...
    for super::PersistentARTrieChar<V, S>
{
    ...
    fn bound_increment_delta(&self, key: &str, delta: u64) -> Result<i64> {
        ...
        i64::try_from(delta).map_err(|_| {
            crate::persistent_artrie::core::error::PersistentARTrieError::InvalidOperation(format!(
