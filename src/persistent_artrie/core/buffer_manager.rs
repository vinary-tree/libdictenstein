...
impl<S: BlockStorage> BufferManager<S> {
    ...
    pub fn try_new(storage: S, pool_size: usize) -> Result<Self> {
        ...
        frames.try_reserve_exact(pool_size).map_err(|source| {
            PersistentARTrieError::allocation_failed(
                "buffer-manager frame metadata",
                pool_size,
                source,
            )
        })?;
        ...
        buffer_pool.try_reserve_exact(pool_size).map_err(|source| {
            PersistentARTrieError::allocation_failed(
                "buffer-manager aligned page pool",
                pool_size,
                source,
            )
        })?;
        ...
        buffers.try_reserve_exact(pool_size).map_err(|source| {
            PersistentARTrieError::allocation_failed(
                "buffer-manager fixed-buffer registration table",
                pool_size,
                source,
            )
        })?;
        ...
        page_table.try_reserve(pool_size).map_err(|source| {
            PersistentARTrieError::allocation_failed("buffer-manager page table", pool_size, source)
        })?;
