...
#[cfg(test)]
mod snapshot_cursor_capability_tests {
    use super::*;
    use crate::persistent_artrie::core::key_encoding::{ByteKey, CharKey, U64Key};

    fn assert_owned_worklist_capability<K: KeyEncoding>() {
        let owner =
            OverlayDictionaryNode::<K, u64>::from_overlay_root(Arc::new(OverlayNode::new()), None);
        assert!(owner.snapshot_root_cursor().is_none());
        ...
        assert!(!owner.supports_efficient_snapshot_cursor_edge_ranges());
        assert!(!owner.supports_snapshot_cursor_values());
    }

    #[test]
    fn persistent_overlay_aliases_select_the_owned_worklist_without_cursor_metadata() {
        assert_owned_worklist_capability::<ByteKey>();
        assert_owned_worklist_capability::<CharKey>();
        assert_owned_worklist_capability::<U64Key>();

        // The vocabulary node alias is the `CharKey, u64` overlay
        // monomorphization checked above; retaining this exact type assertion
        // makes that public alias relationship explicit.
        fn assert_vocab_alias(_: crate::persistent_artrie::vocab::VocabTrieNodeRef) {}
        let vocab = OverlayDictionaryNode::<CharKey, u64>::from_overlay_root(
            Arc::new(OverlayNode::new()),
            None,
        );
        assert_vocab_alias(vocab);
    }
}
