...
mod tests {
    ...
    fn test_optimistic_cell_read_with_retry_fails() {
        ...
        // Try to read with very few retries while writes are active. The exact
        // success/failure mix is scheduler-dependent; this exercises the retry
        // path without pinning a probabilistic count.
        for _ in 0..100 {
            let _ = cell.read_with_retry(|v| *v, 1);
        }

        writer_running.store(false, std::sync::atomic::Ordering::Relaxed);
        writer.join().expect("writer should complete");

        // If all 100 reads succeeded with just 1 retry each while writes are
        // happening, that's also valid behavior.
    }
