...
mod l31_differential_tests {
    ...
    // ---- format-3 (legacy owned StringBucket-suffix; the brick witness) ----
    #[test]
    fn diff_format3_owned_u64() {
        let (_d, mut t) = create::<u64>("l31-f3-u64");
        ...
        assert!(t.insert("member"));
        assert!(t.insert_with_value("", 555));
        t.checkpoint().expect("checkpoint");
        assert_new_eq_oracle(&mut t, "format3-owned-u64");
    }

    #[test]
    fn diff_format3_owned_string() {
        let (_d, mut t) = create::<String>("l31-f3-str");
        t.kill_switch_to_owned();
        for (term, v) in [("alpha", "A"), ("alpine", "B"), ("beta", "C")] {
            assert!(t.insert_with_value(term, v.to_string()));
        }
        assert!(t.insert("bare"));
        t.checkpoint().expect("checkpoint");
        assert_new_eq_oracle(&mut t, "format3-string");
    }

    #[test]
    fn diff_format3_owned_unit() {
        let (_d, mut t) = create::<()>("l31-f3-unit");
        t.kill_switch_to_owned();
        for term in ["red", "redder", "blue", "green"] {
            assert!(t.insert(term));
        }
        assert!(t.insert_with_value("valued", ())); // V=() valued ⇒ Some([]) on disk
        assert!(t.insert("")); // "" membership
        t.checkpoint().expect("checkpoint");
        assert_new_eq_oracle(&mut t, "format3-unit");
    }

    /// A deep key + a branching corpus through the owned (format-3) path — stresses the DFS
    /// path accumulation + recursion (kept moderate so `overlay_eq`'s recursion is stack-safe).
    #[test]
    fn diff_format3_deep_and_branching() {
        let (_d, mut t) = create::<u64>("l31-f3-deep");
        t.kill_switch_to_owned();
        let deep: String = "z".repeat(300);
        assert!(t.insert_with_value(&deep, 1));
        for (term, v) in [("ban", 2u64), ("banana", 3), ("bandana", 4), ("band", 5)] {
            assert!(t.insert_with_value(term, v));
        }
        t.checkpoint().expect("checkpoint");
        assert_new_eq_oracle(&mut t, "format3-deep-branching");
    }
}
