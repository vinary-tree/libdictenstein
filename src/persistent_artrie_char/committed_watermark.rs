...
//! `contiguous` ≙ `Watermark`, [`mark_committed`] ≙ the spec's `Commit(w)` action.
...
/// checkpoint capture); [`mark_committed`] briefly serializes committers under a
/// `Mutex` to close the prefix, but runs **after** the writer's root CAS has
/// already made the write visible — so it is off the contended CAS-retry loop
/// and preserves the lock-free *progress* of visible publication.
#[derive(Debug)]
pub(crate) struct CommittedWatermark {
...
impl CommittedWatermark {
    ...
    pub(crate) fn mark_committed(&self, lsn: u64) {
