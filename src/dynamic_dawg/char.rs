...
impl<V: DictionaryValue> DynamicDawgChar<V> {
    /// Canonical logical profile represented by this Unicode DAWG.
...
impl<V: DictionaryValue> DictionaryNode for DynamicDawgCharNode<V> {
    ...
    where
        F: FnMut(char, Self::SnapshotCursor),
    {
        ...
        Some(unsafe {
            LockFreeDawgNode::<char, V>::visit_cursor_edge_page(cursor, start, capacity, visitor)
        })
    }
    ...
    unsafe fn snapshot_cursor_edge_range_start(
        &self,
        cursor: Self::SnapshotCursor,
    ) -> Option<crate::SnapshotEdgeRangeStart<char, Self::SnapshotCursor, Self>> {
        // SAFETY: inherited from the trait contract; `self` retains the exact
        // immutable revision that owns the cursor and returned storage.
        let native = unsafe { LockFreeDawgNode::<char, V>::cursor_edge_range_start(cursor) };
    ...
    unsafe fn snapshot_cursor_edge_range_step(
        ...
        let (label, child, remaining) =
            unsafe { LockFreeDawgNode::<char, V>::cursor_edge_range_step(native) };
        let remaining = remaining.map(|token| {
            let (current, end) = token.into_raw_parts();
