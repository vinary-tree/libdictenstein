...
impl SegmentSyncManager {
    ...
    pub fn new(
        ...
        Self::with_sync_backend(config, archive_config, wal_path, initial_synced_lsn, Arc::new(StdFsync))
    }

    /// Create a new sync manager with a custom sync backend.
    ///
    /// Use this to inject an `IoUringFsync` backend for io_uring-based fsync,
    /// or a custom backend for testing.
