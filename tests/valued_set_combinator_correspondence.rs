...
mod semiring_correspondence {
    use super::*;
    use libdictenstein::union_zipper::SemiringLatticeWrapper;
    ...
    fn collect_tropical_union(
        maps: &[BTreeMap<String, SemiringLatticeWrapper<TropicalWeight>>],
    ) -> BTreeMap<String, f64> {
        let tries: Vec<DoubleArrayTrie<SemiringLatticeWrapper<TropicalWeight>>> = maps
            .iter()
            .map(|map| {
                DoubleArrayTrie::from_terms_with_values(
                    map.iter().map(|(term, value)| (term.as_str(), *value)),
        ...
        let zippers: Vec<_> = tries
            ...
            .map(DoubleArrayTrieZipper::new_from_dict)
            .collect();
        let union = UnionZipper::with_strategy(zippers, LatticeJoin);
    ...
    fn collect_bool_union(
        maps: &[BTreeMap<String, SemiringLatticeWrapper<BoolWeight>>],
    ) -> BTreeMap<String, bool> {
        let tries: Vec<DoubleArrayTrie<SemiringLatticeWrapper<BoolWeight>>> = maps
            .iter()
            .map(|map| {
                DoubleArrayTrie::from_terms_with_values(
                    map.iter().map(|(term, value)| (term.as_str(), *value)),
    ...
    #[test]
    fn idempotent_semiring_plus_is_union_join_boundary() {
        let maps = vec![
            BTreeMap::from([
                ("path".to_string(), SemiringLatticeWrapper(tropical(5.0))),
                (
                    "inf".to_string(),
                    SemiringLatticeWrapper(TropicalWeight::zero()),
                ),
            ]),
            BTreeMap::from([
                ("path".to_string(), SemiringLatticeWrapper(tropical(3.0))),
                ("inf".to_string(), SemiringLatticeWrapper(tropical(42.0))),
            ]),
            BTreeMap::from([("path".to_string(), SemiringLatticeWrapper(tropical(4.0)))]),
        ];

        assert_eq!(
            collect_tropical_union(&maps),
            BTreeMap::from([("inf".to_string(), 42.0), ("path".to_string(), 3.0)])
        );
    }

    #[test]
    fn boolean_semiring_plus_is_union_join_boundary() {
        let maps = vec![
            BTreeMap::from([
                ("a".to_string(), SemiringLatticeWrapper(BoolWeight(false))),
                ("b".to_string(), SemiringLatticeWrapper(BoolWeight(false))),
            ]),
            BTreeMap::from([
                ("a".to_string(), SemiringLatticeWrapper(BoolWeight(true))),
                ("b".to_string(), SemiringLatticeWrapper(BoolWeight(false))),
            ]),
        ];

        assert_eq!(
            collect_bool_union(&maps),
