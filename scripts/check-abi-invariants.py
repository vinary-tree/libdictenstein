...
Stdlib-only; exit 1 on any failure. Wired into
`scripts/verify-formal-correspondence.sh` and run directly in CI.
"""
...
REGISTRY = ROOT / "formal-verification" / "ABI_INVARIANTS.tsv"
...
def main() -> int:
    ...
    hook_files = sorted((ROOT / "tests").rglob("*.rs")) + [ROOT / "src" / "bindings.rs"]
