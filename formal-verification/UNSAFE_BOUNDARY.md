...
| Shared persistent public concurrency | `persistent_artrie::SharedARTrie`, `persistent_artrie::char::SharedCharARTrie`, `persistent_artrie::vocab::SharedVocabARTrie` | `SharedPersistentConcurrency.tla` checks the bounded byte/char lock-collapse protocol: lock-free write publication, serialized checkpoint publication, post-snapshot writers retained by dirty/WAL evidence, read observation, sync non-publication, and recovery. `SharedPersistentConcurrencySpec.v` proves the corresponding checkpoint, sync, read, and recovery laws. Rust correspondence tests cover byte/char/vocab shared checkpoint/write/sync/reopen races. |
| Raw char child-pointer and vocab reverse-map ownership | `persistent_artrie::char::types`, `persistent_artrie::vocab::{dict_impl,query_api,overlay_serialize}`, and unsafe `Send`/`Sync` surfaces | `CharTrieNodeInner` is an exclusively owned serialization tree: its child store and raw-slot insertion seam are crate-private, the general public `get_child_mut` facade is absent, in-memory child ownership enters only through `insert_child`, removal/replacement detach before `Box::from_raw`, and Drop drains detached child stores iteratively. `PointerOwnership.tla` checks bounded raw slot pointer ownership, borrow exclusivity, unswizzle/drop no-dangling-reference obligations, and no double drop. `VocabPersistenceOwnership.tla` checks stable overlay term indexes across checkpoint/reopen and exact `reverse_term_map` reconstruction. Correspondence tests cover remove/replacement/deep-clone ownership transfer, unique `get_or_create_child` borrows, constrained-stack reclamation, vocab checkpoint/reopen, and duplicate stability. Miri execution is wired through `RUN_MIRI=1`, with strict provenance by default. |
| mmap/io_uring storage access | `persistent_artrie::core::{disk_manager,io_uring_disk_manager}` and trie constructors | `MmapBlockStorage.tla` checks the bounded allocation/remap/access protocol. `StorageSyscallOutcome.tla` checks the bounded write/sync outcome boundary: only full writes followed by successful syncs advance the durable/reported/recovered prefix. `IoUringFixedBufferOwnership.tla` checks fixed-buffer registration, in-flight fixed I/O, unregister, invalid registration, fallback, and owner-drop ordering. `IoUringSqeCqeLifecycle.tla` checks the bounded submit/complete lifecycle: each submitted request owns one live buffer until exactly one CQE is checked, short/error completions fail closed, and temporary buffers are returned only after checking. Storage correspondence tests cover concurrent mmap allocation uniqueness, sub-block bounds rejection, sync/reopen checksum refresh, `raw_ptr` bounds, failed WAL fsync frontier handling, io_uring range rejection, fixed-buffer registration input validation, and unregister-before-owner-drop behavior when the backend is available. Kernel io_uring internals remain trusted implementation code. |
...
| Raw child replacement returns the old `Box` and does not alias the new child. | Char remove/replace/deep-clone correspondence tests, Miri-gated in the harness. |
...
- raw char child slots transfer ownership through remove/replace/deep clone
  without aliasing the returned `Box`; unique `get_or_create_child` mutation
  paths reuse the same child borrow, and the explicit unsafe `Send`/`Sync`
  surface remains type-checked by the correspondence target.
...
- vocabulary checkpoint/reopen preserves stable forward indexes and reverse maps,
  duplicate inserts keep the original index, and `reverse_term_map` is rebuilt
  exactly from the overlay checkpoint image plus retained WAL tail.
...
state transitions, vocab checkpoint/reopen bijection, duplicate stability after
reopen, public read-traversal lazy failure closure, and buffer-manager
fixed-buffer lifetime, and the unsafe inventory gate now rejects
...
4. Keep `BufferPageLease.tla`, `VocabPersistenceOwnership.tla`,
