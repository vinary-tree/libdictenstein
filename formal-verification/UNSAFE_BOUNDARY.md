...
| Raw char child-pointer and vocab reverse-map ownership | `persistent_artrie::char::types`, `persistent_artrie::vocab::{dict_impl,query_api,overlay_serialize}`, and unsafe `Send`/`Sync` surfaces | `PointerOwnership.tla` checks bounded raw slot pointer ownership, disk-slot/lazy-load publication, borrow exclusivity, unswizzle/drop no-dangling-reference obligations, and no double drop. `VocabPersistenceOwnership.tla` checks stable overlay term indexes across checkpoint/reopen and exact `reverse_term_map` reconstruction from the overlay image plus WAL tail. Correspondence tests cover char child remove/replacement/deep-clone ownership transfer, unique `get_or_create_child` mutation borrow paths, vocab checkpoint/reopen bijection, duplicate stability after reopen, and compile-time `Send`/`Sync` contracts. Miri execution is wired through `RUN_MIRI=1`, with `FORMAL_MIRI_TOOLCHAIN=nightly` support and filesystem isolation disabled for filesystem-backed persistence targets. |
