...
(* collapse.                                                                *)
(*                                                                          *)
(* Scope: byte, char, AND vocab shared handles are bare Arc<T> handles.      *)
(* Public writes do not acquire a global RwLock; they linearize when the     *)
(* WAL-backed root publication becomes visible. Public checkpoint uses the   *)
(* internal checkpoint_lock only to serialize checkpoint publication. Reads  *)
(* observe Arc/root snapshots of completed visible states; sync does not     *)
(* publish checkpoint state. SharedVocabARTrie was collapsed to a bare       *)
(* Arc<T> (the vocab F4 lock-collapse) and is now covered by this contract.  *)
(****************************************************************************)
