...
| A4 (SharedVocabARTrie mapped mutation semantics) | 2026-05-21, superseded 2026-06-13 | 7 trait methods silently dropped user input (`insert_with_value` ignored value, `union_with` ignored merge_fn, `update_or_insert` ignored both, `remove`/`remove_prefix`/`upsert` ignored args, `increment` returned `Err`) | Initial hardening made unsupported calls warn instead of silently discarding input. The current implementation has since replaced the mapped-write stubs: `insert_with_value`, `update_or_insert`, and `union_with` now perform real index-aware writes for `PersistentVocabARTrie` and `SharedVocabARTrie`; append-only removal-style methods still return documented sentinel values. | `tests/vocab_trait_honesty.rs`; `tests/persistent_vocab_wal_atomicity_correspondence.rs`; `tests/persistent_vocab_checkpoint_correspondence.rs`; `tests/persistent_shared_concurrency_correspondence.rs`; `tests/dictionary_law_correspondence.rs`; `tests/bijective_trait_invariant.rs` | Current contract: supplied `u64` values are requested vocabulary indices for new terms, existing indices are immutable, missing union terms preserve source indices, and conflicting merge results warn rather than remap a term. |
