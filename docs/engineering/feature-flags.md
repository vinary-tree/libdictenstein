...
| `serialization` | | Serde + bincode binary persistence | `serde`, `bincode-next` | no JSON/TOML/text persistence; see [deserialization-safety](../security/deserialization-safety.md) |
| `compression` | | gzip wrapper over bincode or protobuf bytes | `flate2` | corpus-dependent benefit; decompression-bomb caveat — [deserialization-safety](../security/deserialization-safety.md) |
