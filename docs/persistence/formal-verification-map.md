...
| Invariant | TLA⁺ model | Rocq proof | Doc |
|-----------|-----------|-----------|-----|
| Eviction-CAS overwrite-race, stale-image safety, exact fault provenance, and coordinator-free resident reads | `OverlayEvictionCas.tla`, `OverlayEvictionStale.tla`, `DelayedFaultCoordinatorAcquisition.tla` (+ stale-eviction, fault-stamp, and eager-acquisition unsafe controls) | `Spec/OverlayFaultProvenanceSpec.v`, `tests/persistent_artrie_loom_correspondence.rs`, byte/char coordinator-probe unit tests | [eviction](eviction.md), [concurrency-model](concurrency-model.md) |
...
| Eviction-walk epoch reclamation; lock-free exact-root publication and helped residency; capture-time checkpoint routing; detached callback separation; irreversible coordinator retirement and re-enable | `EvictionWalkEBR.tla`, `EvictionExactRootPublication.tla`, `HelpedRootResidency.tla`, `CapturedCheckpointEvictionRoute.tla`, `DetachedCallbackSeparation.tla` (+ exact-root, helped-residency, capture-route, and capability-separation unsafe controls) | `Spec/PersistentCharEpochReclamationSpec.v`, `Spec/EvictionExactRootPublicationSpec.v`, `Spec/HelpedRootResidencySpec.v`, `Spec/DetachedCallbackSeparationSpec.v`, `tests/persistent_eviction_publication_gate_loom.rs` | [eviction](eviction.md), [concurrency-model](concurrency-model.md) |
| Detached compatibility authority and cacheless owned lookup/removal: total rejection, last-collision selection, read-only lookup, materialize-before-mutation failure atomicity, exact accounting, and detached results | `DetachedCallbackSeparation.tla`, `DetachedCompatibilityInstall.tla`, `CachelessOwnedRegistry.tla` (+ authority-separation, panic/overwrite, first-collision, and mutate-before-materialize controls) | `Spec/DetachedCallbackSeparationSpec.v`, focused unit tests in `src/persistent_artrie/core/eviction/{coordinator,disk_registry}.rs`, `tests/public_eviction_registry_api_correspondence.rs` | [eviction](eviction.md), [concurrency-model](concurrency-model.md) |
...
| Stack-safe overlay serialization specialization: revision-bound arborescence authority, tree/DAG trace equivalence, guarded deferred stamps, zero census work, and optional-cursor selection of an iterative owned worklist without per-handle metadata | `OverlayTreeWitness.tla` (+ forged-DAG, stale-witness, unwitnessed-fast-path, and admitted-cycle unsafe controls) | `Spec/OverlayArborescenceSerializationSpec.v`, `Spec/SerializationRoundtripSpec.v`, focused policy/ownership regressions in `src/persistent_artrie/core/overlay/compressed_serialize.rs`, protobuf V1/V2 parity/deep tests, and `tests/persistent_overlay_hot_path_allocations.rs` | [eviction](eviction.md), [concurrency-model](concurrency-model.md) |
...
| `formal-verification/tla+/AbiSnapshotInitializerTakeover.tla` (+`_SingleConstructionUnsafe.cfg` negative control) | helpably replaceable cold generations preserve immutable producer/revision identity and bound each contender to one construction; pointer identity and globally single construction are not safety requirements | `src/bindings.rs` cold-snapshot takeover and coherent-return unit tests |
