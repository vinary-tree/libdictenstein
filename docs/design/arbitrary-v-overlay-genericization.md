...
**Synthesized in:** [The lock-free overlay — the live representation](../persistence/lock-free-overlay.md). The overlay's immutable `Option<V>` value field and its `K: KeyEncoding` genericity are described there at the architecture level; this record is the mechanism-detail roadmap for lifting that value from the `u64`-only `AtomicU64` to an arbitrary construction-time `Option<V>`, so the lock-free architecture becomes the default for *all* `V` (not just `()` / `u64`-counter).
