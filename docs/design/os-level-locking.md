...
this — Tier-1 *is* the `LOCK_EX` (writer) half of the SWMR lock protocol.
...
The lock is implemented with `rustix::fs::flock` — a **safe** wrapper (no new `unsafe`, so the
formal unsafe-inventory is untouched); `rustix` was already in the lockfile transitively. The
returned guard is stored in the `DiskManager` (`wlock: Option<WLockGuard>`) for the handle's whole
lifetime.
...
`flock` is per **open-file-description**, so a second `open()` of the same file *within one process*
conflicts too — not just cross-process. Two legitimate in-process patterns rely on same-path reopen:
...
A raw `flock` would wrongly reject case 1. The fix is a **process-global refcounted registry**
(`Mutex<HashMap<CanonicalPath, LockEntry>>`) consulted before the `flock`:
...
- **First acquire for a key.** Take the OS `flock` (this is what excludes *other* processes) and
  insert `{ file, refs = 1 }`.
- **Subsequent acquire (same process).** Increment `refs` and return a guard that **shares** the one
  `flock` — no self-conflict.
...
The OS releases a process's `flock`s when it exits (normally or by crash), because all its fds close.
So a crashed owner's lock is freed automatically, and the next process opens + recovers via
...
is gone but its lock is not yet reclaimed". **Single host only**: advisory `flock` is not reliable
across NFS/multi-host, which is out of scope (Tier-2 SWMR is single-host as well).
...
Tier-1 is the `LOCK_EX` (single-writer) half of the SWMR protocol. Tier-2 adds read-only reader
processes holding `LOCK_SH` on a `.rlock` sidecar plus an atomically-`rename`d image and a
background `ArcSwap` refresh — see [`swmr-multiprocess.md`](swmr-multiprocess.md). Because Tier-1
already locks a **stable sidecar**, no Tier-1 change is needed to adopt Tier-2.
