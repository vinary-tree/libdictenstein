...
| [`os-level-locking.md`](os-level-locking.md) | The **Tier-1** exclusive-owner OS advisory lock (Rust `File::try_lock` on a stable `.wlock` sidecar at the `DiskManager` open chokepoints) that makes a second process opening the same file fail cleanly with `FileLocked` instead of silently corrupting it — closing the multi-process footgun on Unix and Windows and forming the exclusive-writer half of SWMR. |
