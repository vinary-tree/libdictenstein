...
(drop). Taken literally, a v1/v2/in-order WAL where NO record is ranked $\Rightarrow$ every record `None` $\Rightarrow$ every record
dropped $\Rightarrow$ **total data loss on the legacy/core path.** BUT §3.2-prose + §3.3 ("in-order $\Rightarrow$ pass `Lsn::MAX`")
...
in-order paths, every unranked record is $\le MAX$ $\Rightarrow$ kept in LSN order = exactly today's
...
the floor claims seqs are subsumed that AREN'T in the image $\Rightarrow$ after reopen the owned image seeds without the
overlay writes, the floor pushes new seqs above them, and reconcile can sort a replayed-overlay-write below a
stale owned-image value $\Rightarrow$ **acked overlay write lost.** D2 §6.3's mitigation is PURE SEQUENCING PROSE ("DG2
...
First half SOUND: §3.2's `Some(s)` arm fires on ANY ranked record regardless of watermark $\Rightarrow$ an acked ranked
write above an unranked hole is NEVER dropped (watermark is a floor for *unranked* drops, not a ceiling for
*ranked* keeps). Converse REAL: §3.2 calls unranked-$\le$-watermark "impossible/anomaly LOSE", but under
...
the caller must be acked ONLY AFTER the rank record is durable** (then acked $\implies$ rank durable $\implies$ never the
trailing-rank case $\implies$ safe). D2 must state "ack-after-rank-sync" as an invariant (and the $\le watermark$-arm-keep
...
CommitRank (and ranks increments) but reconcile keeps reading it as root-version until DG3 $\Rightarrow$ in the
...
comparator-WITH-stamp. **Fix: defer the header $2\to 3$ bump (and thus the v3-comparator selection) to land
...
- **A1c [the headline]** `as_final().store.clone()` cannot lose a child to concurrent tier-growth — the
  ...
  child-add $\Rightarrow$ different node $\Rightarrow$ CAS `expected` mismatch (Arc::ptr_eq) $\Rightarrow$ Conflict $\Rightarrow$ retry re-clones the grown
...
in a retained segment, AFTER `remove("k")`@lsn9, both archived $\Rightarrow$ raw-LSN replay applies Remove(9) then
Insert(11) $\Rightarrow$ **"k" RESURRECTED $\ne$ live-absent.** **D2 §3.3's "archives hold only confirmed records" claim is
...
`(ops, loaded, checkpoint_lsn)`). Runtime `CommittedWatermark` reopens seeded to the FULL frontier $\Rightarrow$ if reconcile
...
assert (`persist.rs:140-146`) only checks `next_lsn` didn't move — NOT that every record $\le$ cp is ranked. §3.3's
`Lsn::MAX`-for-archives conflates "subsumed by a checkpoint that wrote a BASE image" with "present in these
segments"; `recover_from_archives` rebuilds from segments ALONE with no base $\Rightarrow$ archived-but-unranked orphans are
...
(`header.rs:82`); a v2 binary (reverted DG1) refuses a v3 file $\Rightarrow$ user STRANDED. §8 mis-files the one-way bump
...
bypassing it $\Rightarrow$ base-dict write path appends v3 to a v2 file. **Guard must live in `AsyncWalWriter::open_or_create`
...
- **Version-gate the drop rule.** v3: unranked $\Rightarrow$ DROP (every v3 confirmed op is ranked, so unranked = orphan);
  v1/v2: unranked $\Rightarrow$ KEEP in legacy LSN/root-version order (no drop). This subsumes B#1 (the $\le$watermark-keep is the
...
acked $\implies$ ranked $\implies$ never dropped. State explicitly.

**R4 — DG resequencing (B#5/C#5a/C#6): the header $2\to 3$ bump + the new gated reconcile reader MUST land in ONE
