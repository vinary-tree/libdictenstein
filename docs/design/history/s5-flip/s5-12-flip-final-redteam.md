...
`TypeId::of::<V>() ∈ {u64,()}`" and "`enable_lockfree` refuses to stamp Overlay for V $\notin$ {(),u64}" — DOES
...
- **V-1 (CRITICAL, corruption + lock-up): the TypeId gate does not exist.** Wiring `flip_to_overlay()` into
  the generic ctor without it $\Rightarrow$ arbitrary V gets `lockfree_root=Some`+`LockFreeOverlay` $\Rightarrow$ `route_overlay()`
  true $\Rightarrow$ write-broken trie (increment/fetch_add/merge/begin_document reject; batch routes to undefined
...
- **V-2 (HIGH, silent mis-flip): `enable_lockfree` swallows a failed Overlay stamp** (lockfree_cas.rs:186-188
  ...
  enabled with an Owned-regime WAL $\Rightarrow$ recovery KEEPS unranked orphans $\Rightarrow$ resurrection. **FIX:** create-flip
...
- **V-4 (MEDIUM, silent value loss on wrong-V reopen): no on-disk V-type discriminator.** bincode
  trailing-byte tolerance (bincode_compat.rs:57 drops `_consumed`) $\Rightarrow$ `deserialize::<()>([8 u64 bytes])`=Ok(())
...
`flip_to_overlay()` + hard-error if `!took` (fresh WAL $\Rightarrow$ stamp must take). First checkpoint route-splits to
...
- **(a) Overlay-regime + eligible V:** `enable_lockfree()` (idempotent, regime already Overlay $\Rightarrow$ stamp no-op)
  + **V-specialized** reestablish (u64 value-carrying via `…<u64>` extension; `()` membership) + set
  LockFreeOverlay. The `?` ABORTS open on reestablish Err (owned cleared LAST $\Rightarrow$ owned intact). [IRREVERSIBLE]
- **(b) Owned-regime + NON-EMPTY** (old never-flipped trie, new binary): SKIP $\Rightarrow$ stays Owned. Backward-compat:
...
1. **TypeId gate (PRIMARY):** create<u64>/create<()> $\Rightarrow$ route_overlay()==true + MAGIC_OVERLAY; create<String>
   $\Rightarrow$ route_overlay()==false + MAGIC + a subsequent increment/insert_batch succeeds via owned.
2. create→write→reopen (u64 + ()): checkpoint entry_count==N (not 0), reopen all N + correct counts.
3. open-old-Owned-stays-Owned (backward-compat): Owned trie reopen $\Rightarrow$ route_overlay()==false, intact, MAGIC.
...
- **Test A end-to-end:** rendezvous-manufacture a durable UNRANKED orphan: `set_commit_rendezvous(AfterAppend
  ...
  `recover_from_archives` $\Rightarrow$ orphan DROPPED, survivor KEPT. (Core drop already unit-covered by
...
$\Rightarrow$ no torn magic-without-regime window. `from_bytes` accepts both magics, rejects unknown $\Rightarrow$ an old binary
