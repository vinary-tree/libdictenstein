...
function ownedEntry(entry) {
  return Object.freeze([cloneKey(entry[0]), entry[1]]);
}
...
/** Host-owned, stable materialization of one lexicographic dictionary revision. */
export class DictionarySnapshot {
  #entries;

  constructor(entries, ownership) {
    this.#entries = Object.freeze(ownership === owned ? entries : entries.map(ownedEntry));
    Object.freeze(this);
  }
  ...
  const wrap = (dictionary) => {
    const materialize = () => {
      ...
      try {
        ...
        if (exact !== undefined && (!Number.isSafeInteger(exact) || exact < 0)) {
          throw new RangeError("native exact entry count is not materializable");
        }
