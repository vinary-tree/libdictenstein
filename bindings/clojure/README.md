...
| Support tier | Tier 1 |
| Distribution | Clojars `io.vinarytree/libdictenstein-clojure` |
| Native boundary | Idiomatic namespace delegating to the JVM facade |
| Canonical facade source | [`bindings/clojure/src/vinary_tree/libdictenstein.clj`](../../bindings/clojure/src/vinary_tree/libdictenstein.clj) |
...
The canonical checked example is [`bindings/clojure/test/vinary_tree/libdictenstein_test.clj`](../../bindings/clojure/test/vinary_tree/libdictenstein_test.clj). CI runs
the public package path with:

```sh
clojure -M:test -m vinary-tree.libdictenstein-test-runner
```
...
```clojure
(with-open [joined (d/union left right {:value-merge :lattice-join})]
...
Use `with-open`; close persistent stores before process teardown so checkpoints and descriptors are deterministic.
...
JVM failures become `ExceptionInfo` with structured status data. Branch on the typed status or exception, not diagnostic text.
