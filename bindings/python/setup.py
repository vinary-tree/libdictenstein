...
def native_library() -> Path:
    explicit = os.environ.get("LIBDICTENSTEIN_PREBUILT_LIBRARY")
    if explicit:
        library = Path(explicit).expanduser().resolve()
        if not library.is_file():
            raise FileNotFoundError(
                f"LIBDICTENSTEIN_PREBUILT_LIBRARY is not a file: {library}"
            )
        return library
    ...
    target = os.environ.get("LIBDICTENSTEIN_RUST_TARGET")
    if target:
    ...
    library = profile / native_library_name()
    if not library.is_file():
...
class BuildWithNativeLibrary(build_py):
    def run(self) -> None:
        super().run()
        destination = Path(self.build_lib) / "libdictenstein" / "native"
        destination.mkdir(parents=True, exist_ok=True)
        shutil.copy2(native_library(), destination / native_library_name())
        shutil.copy2(
            REPOSITORY_ROOT / "LICENSE",
            Path(self.build_lib) / "libdictenstein" / "LICENSE",
        )
...
setup(
    cmdclass={"build_py": BuildWithNativeLibrary, "bdist_wheel": PortableWheel},
